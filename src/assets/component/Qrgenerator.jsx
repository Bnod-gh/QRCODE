      ...
      <div className="flex flex-col mt-2 ml-2 flex-wrap w-[210mm] h-[297mm] bg-white ">
        {currentPageData.length > 0 ? (
          currentPageData.map((row, index) => (
            <div
              key={index}
              className="w-[70mm] h-[25mm] flex border-[1px] border-dashed items-center"
            >
              <div className="w-[40%] flex items-center justify-center">
                <QRCodeCanvas value={row.suppliercode} size={80} />
              </div>
              <div className="w-3/5 flex flex-col text-[14px]">
                <p className="font-bold h-1/3 overflow-hidden underline">
                  {row.gunzsku}
                </p>
                <p className="h-2/3 overflow-hidden">
                  {row.productname.slice(0, 60)}
                </p>
              </div>
            </div>
          ))
        ) : (
          <p className="flex justify-center items-center pt-8 text-6xl uppercase">
            No Data Available
          </p>
        )}
      </div>
